...
func registerUpdatePOPCheckTool(srv *mcp.Server, h *checksHandler) {
	mcp.AddTool(srv, &mcp.Tool{
		Name:        "update_pop_check",
		Description: "Update an existing POP3 email server monitoring check by ID. Only provided fields are changed.",
	}, h.HandleUpdatePOPCheck)
}

type updatePOPCheckInput struct {
	ID            int64    `json:"id" jsonschema:"check ID"`
	Name          string   `json:"name,omitempty" jsonschema:"display name for the check"`
	Address       string   `json:"address,omitempty" jsonschema:"POP3 server hostname or IP address"`
...
func (c *checksHandler) HandleUpdatePOPCheck(ctx context.Context, _ *mcp.CallToolRequest, in updatePOPCheckInput) (*mcp.CallToolResult, any, error) {
	...
	updated, err := client.Checks().UpdatePOP(ctx, upapi.PrimaryKey(in.ID), check)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to update POP check: %w", err)
	}

	return textResult(fmt.Sprintf("Updated POP check #%d: %s", updated.PK, updated.Name)), nil, nil
}
