...
func registerUpdateTCPCheckTool(srv *mcp.Server, h *checksHandler) {
	mcp.AddTool(srv, &mcp.Tool{
		Name:        "update_tcp_check",
		Description: "Update an existing TCP port connectivity check by ID. Only provided fields are changed.",
	}, h.HandleUpdateTCPCheck)
}

type updateTCPCheckInput struct {
	ID            int64    `json:"id" jsonschema:"check ID"`
	Name          string   `json:"name,omitempty" jsonschema:"display name for the check"`
	Address       string   `json:"address,omitempty" jsonschema:"hostname or IP address to connect to"`
	...
	Port          int64    `json:"port,omitempty" jsonschema:"TCP port number to connect to"`
...
func (c *checksHandler) HandleUpdateTCPCheck(ctx context.Context, _ *mcp.CallToolRequest, in updateTCPCheckInput) (*mcp.CallToolResult, any, error) {
	...
	updated, err := client.Checks().UpdateTCP(ctx, upapi.PrimaryKey(in.ID), check)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to update TCP check: %w", err)
	}

	return textResult(fmt.Sprintf("Updated TCP check #%d: %s", updated.PK, updated.Name)), nil, nil
}
